...
(function defineZonePortal(P) {
  class ZonePortal extends P.Entity {
    constructor({ id, name, x, y, radius, assetKey, nodeAsset, scale, zone }) {
      super({ id, name, x, y, assetKey, blocksMovement: true, collisionRadius: radius || 1.4, label: zone.shortName, scale });
      this.zone = zone;
      this.nodeAsset = nodeAsset;
      this.interactable = true;
      this.tooltip = zone.tooltip;
      this.actionType = "zone";
      this.interactionRadius = 2.6 * (this.scale || 1);
    }

    draw(renderer) {
      renderer.drawSprite(this.assetKey, this.x, this.y, { offsetY: 34, scale: this.scale || 1 });
